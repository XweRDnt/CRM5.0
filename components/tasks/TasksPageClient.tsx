...
export function TasksPageClient({ initialTasks }: TasksPageClientProps): JSX.Element {
  ...
  const { data: tasks, error, isLoading, mutate } = useSWR(`/api/tasks${queryString}`, fetcher, {
    fallbackData: queryString === "" ? initialTasks : undefined,
    revalidateOnMount: queryString !== "",
  });
  ...
  return (
    <section className="space-y-6">
      ...
      {!error && <TaskList tasks={tasks ?? []} loading={isLoading && queryString !== ""} onUpdated={() => void mutate()} />}
    </section>
  );
}

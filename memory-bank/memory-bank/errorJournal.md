...
## [2026-04-07] Проверка сортировки правок блокируется локальной инфраструктурой тестов

**Симптом:** Точечные прогоны vitest для сервиса/публичного портала падают до проверок логики: `Can't reach database server at localhost:5432` и `ECONNREFUSED`.

**Причина:** В текущем окружении не подняты требуемые зависимости для интеграционных тестов (PostgreSQL и API endpoint).

**Решение:** Внесены изменения кода и тестового ожидания по контракту хронологической сортировки; функциональную валидацию выполнить в окружении с поднятой БД и запущенным API.

**Вывод:** Для задач, где нужна проверка порядка выдачи из Prisma/API, заранее поднимать infra или запускать в CI с подготовленным тестовым стеком.
